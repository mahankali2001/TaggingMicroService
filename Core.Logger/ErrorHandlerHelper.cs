...
namespace Core.Logger
{
    ...
    {
        private static readonly ILogger logger = LogManager.GetLogger("WCF");
        ...
        static bool ExceptionInContract(Type serviceType, Exception error)
        {
            ...
            foreach (Type interfaceType in interfaces)
            {
                ...
                {
                    if (methodInfo.Name == serviceMethod)
                     // Does not deal with overlaoded methods 
        ...
        {
            Log entry = CreateLogbookEntry(error, fault);
            try
            {
                logger.Log(error);

                // once the utility to store the error in db / ui we can remove the above line
                WriteToLogFile(entry.ToString(),LogType.Error);
                // When logger exposed as a service this could be used.
        ...
        static Log CreateLogbookEntry(Exception error, MessageFault fault)
        {
            ...
            return new Log(assemblyName, fileName, lineNumber, typeName, methodName, exceptionName, exceptionMessage, providedFault, providedMessage);
        }
        ...
        public static void WriteToLogFile(string p, LogType logType)
        {
            logger.Log(p, logType);
        }

        public static void WriteToLogFile(Exception ex, LogType logType)
        {
            logger.Log(ex);
        }
    }
